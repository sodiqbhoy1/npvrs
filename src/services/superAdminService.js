...
export async function registerSuperAdmin(payload) {
    try{
        const url = `${API_URL}/api/admin/register`;
...
export async function getAllHospitals(token){
    try{
        const url = `${API_URL}/api/admin/hospitals`;
...
export async function approveHospital(hospitalId, token){
    try{
        const url = `${API_URL}/api/admin/approve_hospital`;
        const authToken = token || (typeof localStorage !== 'undefined' && localStorage.getItem('superAdminToken'));
        const headers = authToken ? { Authorization: `Bearer ${authToken}` } : {};
...
export async function getAllPatientsForAdmin(token){
    try{
        const url = `${API_URL}/api/admin/patients`;
        const authToken = token || (typeof localStorage !== 'undefined' && localStorage.getItem('superAdminToken'));
        const headers = authToken ? { Authorization: `Bearer ${authToken}` } : {};
        const {data} = await axios.get(url, { headers });
        return data;
...
export async function forgotPasswordSuperadmin(payload) {
  try {
    const url = `${API_URL}/api/admin/forgot_password`;
    const { data } = await axios.post(url, payload);
    return data;
  } catch (error) {
...
export async function resetPasswordSuperadmin(payload) {
  try {
    const url = `${API_URL}/api/admin/reset_password`;
    const { data } = await axios.post(url, payload);
    return data;
  } catch (error) {
...
// fetches clinic enrollment activity logs from the backend.
export async function getClinicActivityLogs(token){
    try{
        const url = `${API_URL}/api/admin/clinic_stats`;
        const authToken = token || (typeof localStorage !== 'undefined' && localStorage.getItem('superAdminToken'));
        const headers = authToken ? { Authorization: `Bearer ${authToken}` } : {};
        const {data} = await axios.get(url, { headers });
        return data;
