...
export async function registerHospital(payload) {
	try {
    const url = `${API_URL}/api/hospitals/register`;
...
export async function loginHospital(payload){
	try {
		const url = `${API_URL}/api/hospitals/login`;	
		const {data} = await axios.post(url, payload);
		return data;
	} catch (error) {
...
export async function getHospitalProfile(token) {
  try {
    const url = `${API_URL}/api/hospitals/profile`;
...
export async function addPatient(payload, token) {
  try {
	const url = `${API_URL}/api/patients/register`;
